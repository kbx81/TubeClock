...
static enum usbd_request_return_codes _controlRequest(usbd_device *usbd_dev, struct usb_setup_data *req, uint8_t **buf,
...
static void _setConfig(usbd_device *usbd_dev, uint16_t wValue) {
  ...
  usbd_register_control_callback(usbd_dev, USB_REQ_TYPE_CLASS | USB_REQ_TYPE_INTERFACE,
                                 USB_REQ_TYPE_TYPE | USB_REQ_TYPE_RECIPIENT, _controlRequest);
}
...
void initialize() {
  ...
  _usbDev = usbd_init(&st_usbfs_v2_usb_driver, &_devDescriptor, &_cfgDescriptor, _usbStrings, 3, _controlBuffer,
                      sizeof(_controlBuffer));
...
void write(const uint8_t *data, uint16_t len) {
  if (!_connected || _usbDev == nullptr || len == 0 || len > 64) {
    return;
  }
