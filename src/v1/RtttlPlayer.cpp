...
static uint8_t _noteIndex(char c)
{
  switch (c)
  {
    case 'c':           return 1;   // 'c#' is index 2
    case 'd':           return 3;   // 'd#' is index 4
    case 'e':           return 5;
    case 'f':           return 6;   // 'f#' is index 7
    case 'g':           return 8;   // 'g#' is index 9
    case 'a':           return 10;  // 'a#' is index 11
    case 'b': case 'h': return 12;  // German 'h' = English 'b' natural
    case 'p': default:  return 0;   // pause / silence
  }
}
...
static bool _findChar(char c)
{
  ...
  {
    if (_rtttl[_position++] == c) return true;
  }
  return false;
}
...
static bool _parseNote(uint16_t &freq, uint16_t &dur)
{
  ...
  if (scale == 0) scale = _defaultOctave;
...
{
  if (rtttl == nullptr || length == 0) return;

  // Copy (and truncate) the RTTTL string into the playback buffer
  if (length > cRtttlBufferSize) length = cRtttlBufferSize;
  ...
  if (!_findStr2('d', '=')) return;
  uint8_t num = _getInteger();
  if (num > 0) _defaultDuration = num;

  // Parse "o=<n>" (default octave)
  if (!_findStr2('o', '=')) return;
  num = _getInteger();
  if (num >= 3 && num <= 7) _defaultOctave = num;

  // Parse "b=<n>" (BPM)
  if (!_findStr2('b', '=')) return;
  num = _getInteger();
  if (num > 0) bpm = num;
